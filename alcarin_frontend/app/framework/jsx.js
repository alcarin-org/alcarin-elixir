...
import { is, omit, pick, flatten } from 'ramda';
import { CustomComponentContainerClass, JsxComponentDataKey } from './utils';

const SnabbdomModulesAttrs = [
  '$hook',
  '$on',
  '$class',
  '$dataset',
...
export default function jsx(jsxObject) {
  ...
  if (is(Function, jsxObject.elementName)) {
    ...
    const customComponentWrapper = h(
      'div.' + CustomComponentContainerClass,
      snabbdomModules
    );
  ...
  return h(
    ...
    Object.assign(snabbdomModules, { props }),
    children
  );
}
...
function customComponentFactory(customComponentFn, componentContent) {
  const factory = totalState =>
    customComponentFn(
      Object.assign({ $children: componentContent }, totalState)
    );
  // debug only
  Object.defineProperty(factory, 'name', {
    value: customComponentFn.name,
    writable: false,
  });
  return factory;
}
