defmodule Alcarin.GameEventsTest do
  ...
  describe "game_events" do
    ...
    def speak_event_fixture(content \\ "") do
      {:ok, game_event} = GameEvents.create_speak_event(content)
      game_event
    end

    test "list_game_events/0 returns all game_events" do
      check all speak_content_1 <- StreamData.string(:alphanumeric, min_length: 1) do
        game_event = speak_event_fixture(speak_content_1)
        last_listed_event = List.last(GameEvents.list_game_events())
        assert game_event == last_listed_event
      end
    end
